...
function renderApple() {
  const container = document.querySelector('.container__products__items-apple');
  container.innerHTML = '';

  products.appleProducts.forEach((apple) => {
    const appleItem = `
      <div
        role="button"
        onclick="redirectToDetails('${encodeURIComponent(
          apple.title
        )}', '${encodeURIComponent(apple.brand)}', '${encodeURIComponent(
      apple.imgSrc
    )}', '${encodeURIComponent(apple.description)}', '${encodeURIComponent(
      apple.price
    )}', '${encodeURIComponent(apple.rating)}')"
      ...
      >
        ...
        />
        <p class="products__title">${apple.title}</p>
        <p class="products__description">${apple.description}</p>
    ...
    `;
    container.innerHTML += appleItem;
  });
}

document.addEventListener('DOMContentLoaded', renderApple);

// Samsung
function renderSamsung() {
  const container = document.querySelector(
    '.container__products__items-samsung'
  );
  container.innerHTML = '';

  products.samsungProducts.forEach((samsung) => {
    const samsungItem = `
      <div
        role="button"
        onclick="redirectToDetails('${encodeURIComponent(
          samsung.title
        )}', '${encodeURIComponent(samsung.brand)}', '${encodeURIComponent(
      samsung.imgSrc
    )}', '${encodeURIComponent(samsung.description)}', '${encodeURIComponent(
      samsung.price
    )}', '${encodeURIComponent(samsung.rating)}')"
        class="container__products__item"
      >
        <img
          src="${samsung.imgSrc}"
          ...
          class="products__icon"
        />
        <p class="products__title">${samsung.title}</p>
        <p class="products__description">${samsung.description}</p>
        <div class="products__price">
          <p>${samsung.price}</p>
          <a>${samsung.rating}</a>
        </div>
    ...
    `;
    container.innerHTML += samsungItem;
  });
}

document.addEventListener('DOMContentLoaded', renderSamsung);

// Lenovo
function renderLenovo() {
  const container = document.querySelector(
    '.container__products__items-lenovo'
  );
  container.innerHTML = '';

  products.lenovoProducts.forEach((lenovo) => {
    const lenovoItem = `
      <div
        role="button"
         onclick="redirectToDetails('${encodeURIComponent(
           lenovo.title
         )}', '${encodeURIComponent(lenovo.brand)}', '${encodeURIComponent(
      lenovo.imgSrc
    )}', '${encodeURIComponent(lenovo.description)}', '${encodeURIComponent(
      lenovo.price
    )}', '${encodeURIComponent(lenovo.rating)}')"
        class="container__products__item"
      >
        <img
          src="${lenovo.imgSrc}"
          ...
          class="products__icon"
        />
        <p class="products__title">${lenovo.title}</p>
        <p class="products__description">${lenovo.description}</p>
        <div class="products__price">
          <p>${lenovo.price}</p>
          <a>${lenovo.rating}</a>
        </div>
    ...
    `;
    container.innerHTML += lenovoItem;
  });
}

document.addEventListener('DOMContentLoaded', renderLenovo);

// Dell
function renderDell() {
  const container = document.querySelector('.container__products__items-dell');
  container.innerHTML = '';

  products.dellProducts.forEach((dell) => {
    const dellItem = `
      <div
        role="button"
       onclick="redirectToDetails('${encodeURIComponent(
         dell.title
       )}', '${encodeURIComponent(dell.brand)}', '${encodeURIComponent(
      dell.imgSrc
    )}', '${encodeURIComponent(dell.description)}', '${encodeURIComponent(
      dell.price
    )}', '${encodeURIComponent(dell.rating)}')"
        class="container__products__item"
      >
        <img
          src="${dell.imgSrc}"
          ...
          class="products__icon"
        />
        <p class="products__title">${dell.title}</p>
        <p class="products__description">${dell.description}</p>
        <div class="products__price">
          <p>${dell.price}</p>
          <a>${dell.rating}</a>
        </div>
    ...
    `;
    container.innerHTML += dellItem;
  });
}

document.addEventListener('DOMContentLoaded', renderDell);
